...
for it in range(100):
  ...
  res = []
  for i in range(1000):
      p = keypointsLeft[int(random.randrange(0, 1500))].pt ; px = int(p[0]) ; py = int(p[1])
      depth0 = (focale * baseline) / disp0[py][px]
      depth1 = (focale * baseline) / disp1[py][px]
      if(depth0 < 10):
  ...
  for i in range(y):
     for j in range(x):
         ...
         x0 = int(j - 635) ; x1 = int((depth0/depth1)*x0)
         y0 = int(187 - i) ; y1 = int((depth0/depth1)*y0)
         difX = x1 - x0
         difY = y1 - y0
  ...
  #cv2.imwrite('output' + os.path.sep + 'flow2.0' + os.path.sep + '0%09d.jpg'%it, flowCorrect)
