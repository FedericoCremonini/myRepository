...
for i in range(100):
  ...
  keypoints, descriptors = orb.detectAndCompute(left, None)
  kpleft = cv2.drawKeypoints(left, keypoints, None)

  collage0 = np.concatenate((left,disp0, kpleft),1)
  collage1 = np.concatenate((flow_color.astype(np.uint8),disp1, kpleft),1)
